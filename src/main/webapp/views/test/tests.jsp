...
<head>
    ...
    <body>
        ...
        <strong><c:out value="${param.sort}">abc</c:out></strong><br>
        ...
        <form action="/epam/tests" method="GET">
            ...
            <select name="sort" id="sort" onchange="submit()">
                <option value="name ASC" ${param.sort == 'name ASC' ? 'selected' : ''}>///Name ascending</option>
                <option value="name DESC" ${param.sort == 'name DESC' ? 'selected' : ''}>///Name descending</option>
                <option value="scale ASC" ${param.sort == 'scale ASC' ? 'selected' : ''}>///Complexity ascending</option>
                <option value="scale DESC" ${param.sort == 'scale DESC' ? 'selected' : ''}>///Complexity descending</option>
                <option value="questionsNum ASC" ${param.sort == 'questionsNum ASC' ? 'selected' : ''}>///Number of questions ascending</option>
                <option value="questionsNum DESC" ${param.sort == 'questionsNum DESC' ? 'selected' : ''}>///Number of questions descending</option>
            </select>
            <label for="sort">Sorting:</label>
            <select name="sort" id="sort" onchange="submit()">
                <option value="name ASC" ${param.sort == 'name ASC' ? 'selected' : ''}>///Name ascending</option>
            ...
            <ul class="pagination justify-content-center">
                <c:if test="${param.page eq 1}">
                    <li class="page-item disabled">
                        <a class="page-link bg-light text-dark" href="#" tabindex="-1" aria-disabled="true">Previous</a>
                    </li>
                </c:if>
                <c:if test="${param.page gt 1}">
                    <li class="page-item">
                        <a class="page-link bg-dark text-light" href="/epam/tests?page=${param.page-1}&sort=${param.sort}">Previous</a>
                    </li>
                </c:if>
                <c:if test="${param.page eq lastPage}">
                    <li class="page-item disabled">
                        <a class="page-link bg-light text-dark" href="#" tabindex="-1" aria-disabled="true">Next</a>
                    </li>
                </c:if>
                <c:if test="${param.page lt lastPage}">
                    <li class="page-item">
                        <a class="page-link bg-dark text-light" href="/epam/tests?page=${param.page+1}&sort=${param.sort}">Next</a>
                    </li>
                </c:if>
            </ul>
        </nav>
